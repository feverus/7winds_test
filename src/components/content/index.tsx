...
function Content() {
	...
	useEffect(() => {		
		console.log('обновилась переменная dataStore.table')      

        //api.initRowsProps(dataStore.table, 0, dataStore.table.length)
		//api.getGrid(dataStore.table, 0, dataStore.table.length)
	}, [dataStore.table])
	...
	const valueViewer = (cell: GridElement) => {
		const result = (cell.col===0)?
			...
			haveChild={!!dataStore.table[cell.row as number].haveChild} lastChild={!!dataStore.table[cell.row as number].lastChild} />:
			cell.value	
		return result		
	}
